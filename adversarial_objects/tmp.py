...
if __name__ == '__main__':
    ...
    base_object.vertices /= 12.0#6.0 #0.5 #2.0 #
    ...
    for i in range(args.max_iterations):
        ...
        random_elevation = np.random.uniform(0, args.training_range//2+5)
        renderer.eye = nr.get_points_from_angles(camera_distance, random_elevation, azimuth)
    ...
    adv_labels = []
    loop = range(90 - 2*args.validation_range, 90 + 2*args.validation_range, 2)
    ...
    for top_counts in [1,2,3,4,5]:
        for num_elevation, elevation in enumerate(elevation_loop):
            for num, azimuth in enumerate(loop):
                ...
                y_adv_label = torch.topk(y_adv,top_counts)[1].detach().cpu().numpy()
                y_raw_label = torch.topk(y_raw,top_counts)[1].detach().cpu().numpy()
                # pdb.set_trace()
        ...
        if args.target_class > -1:
            print("Target attack on {}: {}, {} out of {} times ".format(
                ...
                correct_target,
                len(adv_labels)))
            loss_handler['targeted_attack'][top_counts].append((correct_target/(0.0+len(adv_labels))))
        loss_handler['raw_accuracy'][top_counts].append((correct_raw/(0.0+len(loop))))
        loss_handler['attack_accuracy'][top_counts].append(((correct_raw-correct_adv)/(0.0+correct_raw)))
        loss_handler['correct_raw'][top_counts].append(((correct_raw)))
        loss_handler['correct_adv'][top_counts].append(((correct_adv)))
        loss_handler['correct_target'][top_counts].append(correct_target)
        loss_handler.log_epoch(writer_tf, top_counts)
